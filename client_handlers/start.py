...
class StartCmd(BaseHandler):
    ...
    async def func(self):
        ...
        await self.request.reply(
            (
                f"Привет! Я **Церберус**, бот для напоминаний.\n##================##\n**Ближайшее сообщение на сегодня:**\n"
                f"{'У вас не запланировано напоминаний на сегодня' if n_for_cur_user is None else await render_notification(n_for_cur_user)}"
            ),
